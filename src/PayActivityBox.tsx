...
const PayActivityBox = (props: PayActivityBoxProps) => {
    ...
    const loadInvoice = useCallback(async () => {
        if (props.loadDebitNotes) {
            const response = await backendFetch(backendSettings, `/payment-api/v1/payActivity/${props.payActivity.id}/invoice`);
            const response_json = await response.json();
    ...
    return (
        <div className={"pay-activity-box"}>
            <div className={"pay-activity-box-body"}>
                <div className={"pay-activity-id"}>Amounts for activity with
                    id <b>{props.payActivity.id}</b> (Role: {props.payActivity.role}):
                </div>
